...
locals {
  ...
  root_rotate_hrs = (var.root_ca_validity_period_hours - var.rotate_root_ca_hours_before_expiry) > 0 ? (var.root_ca_validity_period_hours - var.rotate_root_ca_hours_before_expiry) : 1
}

resource "time_rotating" "rotate_root" {
  rotation_hours = local.root_rotate_hrs
}
